...
const ArrowIcon = ({ isOpen }: { isOpen: boolean }) => (
	<svg
		...
		height="1em"
		viewBox="0 -6 524 524"
		style={ { marginLeft: '5px', transform: isOpen ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.2s' }}
	>
	...
	return (
		<button
			...
			className={`
				...
				bg-primary-blue-900 border-neutral-500
				${isOpen ? 'shadow-inner' : ''}
				${className ?? ''}
			`}
	...
	(props, ref) => {
		...
		return (
			<ul
				...
				ref={ref}
				role="menu"
				className={`absolute left-0 top-full pt-2 mt-1.5 z-50 min-w-60 flex flex-col gap-2 px-2 py-2 border border-neutral-500 shadow-lg rk:border-zinc-800/50 bg-primary-blue-900 ${className ?? ''}`}
				{...rest}
			>
				{children}
			</ul>
